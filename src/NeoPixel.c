...
//findes de allerede? #define cli() asm volatile("cli"::)
//findes de allerede? #define sei() asm volatile("sei"::)
...
*//*
void NeoPixel_Init(uint16_t n, uint8_t p, neoPixelType t) *//*:
...
*/
 ...
 //#warning"Disabled check for to fast re-show (NeoPixel)"
  bool           NeoPixel_canShow(void) { return true;/*(micros() - endTime) >= 300L; */}
...
#if defined(PORTB)
 #if defined(PORTE) || defined(PORTC) || defined(PORTF)
    if(port == &PORTB) {
 #endif // defined(PORTE/C/F)
...
#if defined(PORTC)
 #if defined(PORTE) || defined(PORTB) || defined(PORTF)
    if(port == &PORTC) {
 #endif // defined(PORTE/B/F)
...
#if defined(PORTF)
 #if defined(PORTE) || defined(PORTB) || defined(PORTC)
    if(port == &PORTF) {
 #endif // defined(PORTE/B/C)
...
*/
void NeoPixel_setPin(volatile unsigned char *LED_ControlPort, uint8_t LED_ControlPin) 
{
  /*if(begun && (pin >= 0)) NeoPixel_pinMode(pin, INPUT);
    pin = p;
    if(begun) 
	{
      pinMode(p, OUTPUT);
...
//    port    = portOutputRegister(digitalPinToPort(p));
    port    = LED_ControlPort;
	*port &= ~(1 << LED_ControlPin); // make sure pin starts at low
//    pinMask = digitalPinToBitMask(p);
    pinMask = 1 << LED_ControlPin;
#endif
}
